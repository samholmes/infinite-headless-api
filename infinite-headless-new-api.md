---
title: 'Headless SDK'
description: 'Complete guide to integrating wallet-based authentication and on-ramp/off-ramp functionality'
---
...
- **Have Wallet Auth Enabled**: The `WalletAuthEnabled` flag must be set to true for the organization
...
- **publicKey**: `string` (required) - Ethereum wallet public key (address)
...
- **public_key**: `string` (required) - Ethereum wallet public key
- **signature**: `string` (required) - Signature from wallet signing the challenge message
- **nonce**: `string` (required) - Challenge nonce from the challenge request
- **platform**: `string` (optional) - Platform making the request (e.g., "web", "ios", "android")
- **domain**: `string` (optional) - Domain used for EIP-4361 (SIWE)
- **message**: `string` (optional) - Message that was signed (for verification)
...
- **No authentication required** - Can be accessed before wallet authentication
...
- **type**: `string` (required) - "INDIVIDUAL" or "BUSINESS" (case-insensitive)
...
- **companyInformation**: `object` (required for BUSINESS)
...
- **redirectUrl**: `string` (required) - URL to redirect to after KYC completion
...
- **type**: `string` (required) - "bank_account"
...
- **fees**: Fee breakdown
  - **infiniteFee**: Fee charged by Infinite
  - **edgeFee**: Fee charged by Edge
  - **total**: Total fees
  - **currency**: Currency of the fees
...
```http
GET /v1/headless/customers/{customerId}
Authorization: Bearer eyJhbGciOiJIUzI1NiIs...
X-Organization-ID: your_organization_id
```
